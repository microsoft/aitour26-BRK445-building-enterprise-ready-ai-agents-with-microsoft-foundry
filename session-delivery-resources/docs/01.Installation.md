...
- Aspire CLI — Install with `curl -sSL https://aspire.dev/install.sh | bash` (Linux/macOS) or `iex "& { $(irm https://aspire.dev/install.ps1) }"` (PowerShell/Windows). Verify with `aspire --version`. See https://aspire.dev/get-started/install-cli/ for details.
...
- MafAIFoundry (`maf_ai_foundry`) — variant targeting Microsoft Foundry-hosted agents (AIFoundry variant).
...
**Navigation:** [⬅ Previous: Prerequisites](./Prerequisites.md) | [🏠 Session Delivery Resources](../readme.md) | [Next: Needed Cloud Resources ➡](./02.NeededCloudResources.md)