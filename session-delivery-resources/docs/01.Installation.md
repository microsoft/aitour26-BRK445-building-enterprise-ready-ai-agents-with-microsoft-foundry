...
Before running the demo locally you must provision a small set of cloud resources one time only.

**Automated (recommended):** Run the setup script from the `infra/` folder to provision all Azure resources and configure .NET user secrets in one step:
...
Then deploy agents and run the demo:

```bash
cd infra && dotnet run    # Deploy agents to Foundry
cd src && aspire run       # Start the application
```

**Manual:** See the detailed guide here: [02 — Needed Cloud Resources](./02.NeededCloudResources.md) for step-by-step portal instructions.
