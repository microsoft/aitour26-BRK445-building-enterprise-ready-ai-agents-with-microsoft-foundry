...
The fastest way to provision all Azure resources and configure your environment is the `setup_azure.py` script. It handles infrastructure, AI Services, model deployments, and .NET user secrets in one run.
...
After it completes, deploy the agents and run the demo:
...
If you already deployed infrastructure (e.g., via `deploy_azure_resources.py`), skip that phase:

```bash
python setup_azure.py --skip-infra
```

See `infra/AZURE_DEPLOYMENT_README.md` for full options and details.

> If you prefer to set up resources manually via the Azure portal, follow the step-by-step instructions below.
...
![Resource Group with Application Insights and Microsoft Foundry resources](./imgs/20-ResourceGroupWithAppInsAndFoundry.png)
